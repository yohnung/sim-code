...
int main()
{
	...
	for (nstep=nstart;nstep<nend;nstep++)
	{
		...
		set_eta(eta, var, current, system_time);            // Setting space dependent conductivity. 
		                                                    // (Can make it depend on current)	
		...
		dt=set_dt(var, eta, current, p, system_time);       // Settiing appropriate time-interval from main variables, conductivity
	                                                        // and pressure. This statement doesn't change pressure.	
		step_on(var, flux, system_time, dt);                // Main procedure to time step on variables from Flux explicitly 
		                                                    // and from Source implicitly.
		if (nstep%3==0)
			smooth(var,system_time, nstep);                 // 这个函数没有看懂！
		system_time=system_time+dt;		
		timeout<<" "<<nstep<<setprecision(3)<<setiosflags(ios::fixed)<<system_time;	
